...
class DatabaseService {
  ...
  Stream<List<Student>> getStudents({
    ...
    return query.snapshots().map((snapshot) {
      ...
      if (classificationFilter != null && classificationFilter != 'Tất cả') {
        students = students
            .where((s) => s.classification == classificationFilter)
            .toList();
      }
  ...
  Future<void> addClass(ClassModel classModel) =>
      _classesRef.add(classModel.toMap());
  Future<void> updateClass(ClassModel classModel) =>
      _classesRef.doc(classModel.id).update(classModel.toMap());
  ...
  // --- Department CRUD ---
  Future<void> addDepartment(Department dept) =>
      _departmentsRef.add(dept.toMap());
  Future<void> updateDepartment(Department dept) =>
      _departmentsRef.doc(dept.id).update(dept.toMap());
    ...
    return _studentsRef.snapshots().map((snapshot) {
      ...
      if (total == 0) {
        return {
          ...
          'classificationStats': <String, int>{
            'Xuất sắc': 0,
            'Giỏi': 0,
            'Khá': 0,
            'Trung bình': 0,
      ...
      for (var doc in snapshot.docs) {
        ...
        classificationStats[classification] =
            (classificationStats[classification] ?? 0) + 1;
      }
